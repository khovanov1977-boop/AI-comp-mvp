...
class ImageProviderTestCase(unittest.TestCase):
    ...
    def test_prompts_use_compiled_values_and_omit_unspecified_attributes(self):
        ...
        self.assertIn("Only these fitted items; body proportions visible", body)
    ...
    def test_full_body_face_adjustment_follows_explicit_choice(self):
        for body_type in ("full", "fat"):
            with self.subTest(body_type=body_type):
                ...
                self.assertIn("keep eyes, nose and mouth unchanged", allow)
                self.assertNotIn("facial shape", allow)
                self.assertIn("Keep their face and facial shape", preserve)
