...
class AppearanceTestCase(unittest.TestCase):
    ...
    def test_previous_face_variants_remain_available_and_restore_settings(self):
        self.configure({"gender": "female", "hair_color": "red", "glasses": False})
        first = self.add_candidate("face")
        self.assertEqual(self.choose("face", first).status_code, 200)
        for color in ("blond", "black", "brown", "blue"):
            self.assertEqual(self.configure({"gender": "female", "hair_color": color}).status_code, 200)
            self.assertEqual(self.choose("face", self.add_candidate("face")).status_code, 200)
        before = self.read()
        self.assertEqual(len([item for item in before["candidates"] if item["stage"] == "face"]), 5)
        self.assertFalse(next(item for item in before["candidates"] if item["id"] == first)["current"])
        result = self.choose("face", first)
        self.assertEqual(result.status_code, 200, result.text)
        self.assertEqual(result.json()["settings"], {"gender": "female", "hair_color": "red", "glasses": False})
        self.assertEqual(result.json()["selections"]["face"], first)
        self.assertEqual(self.publish().status_code, 200)

    def test_previous_body_restores_its_face_reference_and_preserves_published(self):
        self.configure({"gender": "female", "hair_color": "red", "body_type": "fit"})
        old_face = self.add_candidate("face")
        self.assertEqual(self.choose("face", old_face).status_code, 200)
        old_body = self.add_candidate("body")
        self.assertEqual(self.choose("body", old_body).status_code, 200)
        self.assertEqual(self.publish().status_code, 200)
        published = deepcopy(self.read()["published"])
        self.configure({"gender": "female", "hair_color": "black", "body_type": "fat", "face_adjustment": "allow"})
        new_face = self.add_candidate("face")
        self.assertEqual(self.choose("face", new_face).status_code, 200)
        self.assertEqual(self.choose("body", self.add_candidate("body")).status_code, 200)
        result = self.choose("body", old_body)
        self.assertEqual(result.status_code, 200, result.text)
        value = result.json()
        self.assertEqual(value["settings"], {"gender": "female", "hair_color": "red", "body_type": "fit"})
        self.assertEqual(value["selections"]["face"], old_face)
        self.assertEqual(value["selections"]["body"], old_body)
        self.assertEqual(value["published"], published)
        self.assertEqual(self.publish().status_code, 200)

    def test_previous_clothing_restores_prior_references(self):
        self.configure({"gender": "female", "body_type": "ordinary", "clothing": "Blue dress"})
        old_face = self.add_candidate("face")
        self.assertEqual(self.choose("face", old_face).status_code, 200)
        old_body = self.add_candidate("body")
        self.assertEqual(self.choose("body", old_body).status_code, 200)
        old_clothing = self.add_candidate("clothing")
        self.assertEqual(self.choose("clothing", old_clothing).status_code, 200)
        self.configure({"gender": "female", "body_type": "athletic", "clothing": "Red coat"})
        self.assertEqual(self.choose("face", self.add_candidate("face")).status_code, 200)
        self.assertEqual(self.choose("body", self.add_candidate("body")).status_code, 200)
        self.assertEqual(self.choose("clothing", self.add_candidate("clothing")).status_code, 200)
        result = self.choose("clothing", old_clothing)
        self.assertEqual(result.status_code, 200, result.text)
        value = result.json()
        self.assertEqual(value["settings"], {"gender": "female", "body_type": "ordinary", "clothing": "Blue dress"})
        self.assertEqual(value["selections"], {"face": old_face, "body": old_body, "clothing": old_clothing})
        self.assertEqual(self.publish().status_code, 200)
    ...
    def test_upstream_edit_during_generation_keeps_output_but_marks_it_stale(self):
        ...
        restored = self.choose("face", candidate["id"])
        self.assertEqual(restored.status_code, 200, restored.text)
        self.assertEqual(restored.json()["settings"], {"gender": "female"})
        self.assertEqual(restored.json()["selections"]["face"], candidate["id"])
