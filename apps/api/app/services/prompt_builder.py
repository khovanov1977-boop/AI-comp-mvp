...
def build_provider_prompt(context: OrchestratorContext) -> ProviderPrompt:
    ...
    system_sections = [
        ...
        "Structured reply contract (highest priority):",
        "- Return only the object required by the response schema. Never wrap it in Markdown or add text outside it.",
        "- Build segments in the exact order in which they occur in the reply.",
        "- kind=speech contains only words the character audibly says. Never put narration, speaker labels, actions, or delivery descriptions in a speech segment.",
        "- kind=action contains only the character's visible action or nonverbal reaction. Do not put spoken words in an action segment.",
        "- kind=thought contains only a private, unspoken thought. kind=scene_note contains scene narration. kind=ooc contains only an out-of-character answer.",
        "- Use kind=thought only when the user used thought notation or explicitly asked for the character's private thoughts. Otherwise say a communicative emotional phrase aloud as kind=speech or omit it.",
        "- When current_reply_delivery is voice, do not use kind=thought. Every meaningful phrase addressed to the user must be kind=speech so it is included in the audio.",
        "- Segment text never includes formatting wrappers such as *, **, ~, [scene:], or ((OOC:)); the application renders those wrappers.",
        "- audio_cue may be used on speech or action segments and describes a sound or vocal delivery at that position: laugh, giggle, sigh, gasp, cough, cry, whisper, short_pause, smile, or none. Thoughts, scene notes, and OOC segments must use audio_cue=none.",
        "- A physical action such as moving, looking, touching, sitting, or holding something normally has audio_cue=none. Never turn a physical action into spoken narration.",
        "- delivery describes how the whole reply should sound. Infer emotion, pace, and intensity from the character, current state, scene, and this conversational turn; prefer subtle or balanced intensity in ordinary conversation.",
        "- In plain chat, normally use speech segments only. Add an action or audible cue only when it contributes naturally to the response; do not decorate every reply.",
        ...
        f"- current_reply_delivery: {'voice' if context.voice_reply_requested else 'text'}",
        ...
        "- The application displays speech as plain text, actions as *action*, private thoughts as ~thought~, scene notes as [scene: note], and out-of-character notes as ((OOC: note)). Express that distinction through segment kinds, not by writing the wrappers yourself.",
        "- Do not force roleplay narration into ordinary plain chat. When the user uses roleplay notation, mirror the same communication mode while keeping speech, actions, and thoughts in separate segments.",
        ...
        "- If detected_response_mode is ooc_only, return only kind=ooc segments and do not advance the scene. If OOC and in-character content are mixed, keep the OOC answer in separate ooc segments and continue only the in-character part requested.",
        "User context:",
        ...
        *(memory_lines or ["- none"]),
        ...
        "- Voice delivery is supported by the application backend. When the user asks for a voice message or asks you to say something aloud, write the requested content immediately as a natural spoken reply.",
        "- Never claim that you cannot send, record, generate, or use a voice message. Never mention technical limitations, text-only mode, audio generation, TTS, the backend, or the delivery mechanism.",
        "- Do not promise to send audio later and do not substitute a description of what you would say. Simply answer now; the application will deliver the reply in the correct format.",
        "- Any earlier assistant statement in recent messages claiming that voice delivery is unavailable was a technical mistake, not a character fact. Do not repeat or preserve it.",
        ...
        "- When writing Russian, use the letter ё where the word or name requires it, especially in names and direct-address forms, so pronunciation is unambiguous.",
